...
### 2.2 カラーパレット
70年代風「モダンレトロ」カラー（Webプロトタイプにて策定）

| 用途 | カラーコード | 説明 |
|------|-------------|------|
| ベース/背景 | #F4E4BC | 質感のあるクリームベージュ |
| アクセント1 | #D97D54 | テラコッタ/バーントオレンジ（再生ボタン） |
| アクセント2 | #87A8A4 | セージ/ティールグリーン |
| テキスト/筐体 | #35312C | ダークブラウン/ブラック |
| 金属部分 | #C0B9B1 | ブラッシュドメタル |
...
### 3.5 コントロールボタン
- **配置**: カセットの下部、筐体下側に埋め込まれる形（ピアノキースタイル）
- **種類**:
  - 再生ボタン（▶）: アクセントカラー（テラコッタ）を使用
  - 停止ボタン（■）: ニュートラルカラー（グレー）
  - 設定ボタン（⚙）: ニュートラルカラー（グレー）
- **デザイン**: ピアノの鍵盤のような長方形スタイル
- **インタラクション**:
  - **モーメンタリ動作**: 押している間だけ沈み込み、指を離すと戻る（実際の動作状態はLED等の別の手段で示すか、ボタン自体は戻るが機能は保持する）
  - **3Dアニメーション**: ボタンの上部を支点として、手前側が沈み込むような物理的な回転挙動を再現
- **フィードバック**:
  - 視覚: 3D回転アニメーション + 影の変化
  - 触覚: バイブレーション（Haptics）
